...
When possible, a fully-owned configuration file makes more sense.  I.e., let Sprinkle, Puppet, Chef, whatever fully own a configuration file and make the changes in the configuration management tool, rather than appending the file.  This approach can easily break if:
...
The problem with this approach (sometimes) is:

* limited regular expression support
* less error control
* manual config history managment

Yet, I prefer the simpler bash approach except for really important config files that I have to touch.
...
From the command line:
  ...
  appender --file /etc/syslog-ng/syslog-ng.conf --remove f_cron --append "destination df_cron { file(\"/var/log/cron.log\"); };/
  filter f_cron {facility(cron); };"
